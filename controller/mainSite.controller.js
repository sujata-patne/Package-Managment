                        ...
                        if (mainSitePackageData != null && mainSitePackageData.length > 0) {
                            async.parallel({
                                ...
                                valuePackPlans: function (callback) {
                                    valuePackManager.getValuePackPlansByStoreId(connection_ikon_cms, req.session.package_StoreId,req.body.distributionChannelId, function (err, valuePackPlans) {
                                        callback(err, valuePackPlans);
                                    });
                                },
                                subscriptionPackPlans: function (callback) {
                                    subscriptionPackManager.getSubscriptionDetailsByStoreId(connection_ikon_cms, req.session.package_StoreId, req.body.distributionChannelId,function (err, valuePackPlans) {
                                        callback(err, valuePackPlans);
                                    });
                                },
                                alacartNOfferDetails: function (callback) {
                                    async.waterfall([
                                        function (callback) {
                                            alacartManager.getAlacartNOfferDetails(connection_ikon_cms, mainSitePackageData[0].sp_pkg_id, function (err, alacartNOfferDetails) {
